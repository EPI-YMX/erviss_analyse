...
get_erviss_variants_url <- function(use_snapshot = FALSE, snapshot_date = NULL) {
 if (use_snapshot) {
    if (is.null(snapshot_date)) {
      stop("'snapshot_date' is required when use_snapshot = TRUE")
    }
    assert_date(snapshot_date, "snapshot_date")
    paste0(ERVISS_BASE_URL, "/snapshots/", format(snapshot_date, "%Y-%m-%d"), "_variants.csv")
  } else {
...
#'
#' # Get snapshot URL
#' get_erviss_positivity_url(use_snapshot = TRUE, snapshot_date = as.Date("2023-11-24"))
get_erviss_positivity_url <- function(use_snapshot = FALSE, snapshot_date = NULL) {
  if (use_snapshot) {
    if (is.null(snapshot_date)) {
      stop("'snapshot_date' is required when use_snapshot = TRUE")
    }
    assert_date(snapshot_date, "snapshot_date")
    paste0(ERVISS_BASE_URL, "/snapshots/", format(snapshot_date, "%Y-%m-%d"), "_sentinelTestsDetectionsPositivity.csv")
  } else {
